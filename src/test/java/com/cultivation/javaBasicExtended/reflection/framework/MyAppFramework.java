...
public class MyAppFramework {
    ...
    public Response getResponse(String requestController, String requestMethod) throws IllegalAccessException, InstantiationException, InvocationTargetException {
        // TODO: Please implement the method
        // <--start
        Class aClass = controllerList.stream().filter(c -> c.getName().equals(requestController)).findFirst().orElse(null);
    ...
    private Method getMethod(String requestMethod, Class aClass) {
        ...
        Method method = Arrays.stream(declaredMethods).filter(d -> d.getName().equals(requestMethod)).findFirst().orElse(null);
        if (Objects.isNull(method)) {
            return null;
        }
        return method;
    ...
    private Response getResult(Object instance, Method method) {
